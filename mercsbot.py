...
async def on_message(message):
    ...
    if message.content.startswith('!resetmsg'):
        embedVar = discord.Embed(title="HP", description="This shows the HP of OUR structures! *and fuck jacob*",
                                 color=0x00ff00)
        ...
        embedVar.add_field(name="Ogre Timer (may differ up to 3 secs)", value=datetime.timedelta(seconds=ogre),
                           inline=False)
...
async def test():
    ...
    embedVar.add_field(name="Guard Tower 2", value=gt2, inline=False)
    embedVar.add_field(name="Holy Artifact", value=ha, inline=False)
    embedVar.add_field(name="Ogre Timer (may differ up to 3 secs)", value=f'{ogremin} minutes and {ogresec} seconds',
                       inline=False)
    await message2.edit(embed=embedVar)
